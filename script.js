...
(function(){
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadApprovedReviews() {
      const apiToken = 'YOUR_NETLIFY_PERSONAL_ACCESS_TOKEN';
      ...
      try {
        ...
        submissions.forEach(sub => {
          const name = sub.data.name || 'Anonymous';
          const book = sub.data.book || '';
          const reviewText = sub.data.review || '';
          // Optionally filter only approved submissions by checking a status field
          if (reviewText) {
            const newSlide = document.createElement('div');
            newSlide.className = 'testimonial';
            const p = document.createElement('p');
